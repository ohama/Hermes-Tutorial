...
초기 릴리스 범위. 각 요구사항은 로드맵 페이즈에 매핑된다. (모든 본문은 한국어, 코드·명령어는 원문 유지. `display.language: ko`는 부분 지원 — 승인 프롬프트 등 일부 정적 메시지만 번역되고 에이전트 응답·로그·도구 출력·오류는 영어이므로 이 튜토리얼은 영어 UI 기준으로 설명한다.)
...
| 완전한 한국어 UI (응답·로그·도구 출력) | `display.language: ko`는 정적 메시지 한정 부분 지원 — 완전 한국어화는 범위 밖 |
